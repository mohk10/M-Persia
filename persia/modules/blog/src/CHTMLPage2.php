...
//require_once(TP_MODULESBLOG.'config2.php');
...
$aTitle=str_replace(" ","%20",$aTitle);
 if(is_null($aImage)||($aImage=="")){
	...
	$html.="<img src='{$aImage}'  alt='bild' />{$aText}</p>";
}

$html.=<<<EOD
<p><strong>Taggar:</strong> {$aTags}</p>
<a href='?m=blog&amp;p=gorakommentar&amp;idComment={$aId}&amp;titleComment={$aTitle}&amp;numbersNote={$aNumbers}'>Kommentera</a><br />
<a href='?m=blog&amp;p=visainlaggkommentar&amp;idComment={$aId}'>Visa kommentarer</a><br /><br />
...
$html.=<<<EOD
<p><strong>Taggar:</strong> {$aTags}</p>
<a href='?m=blog&amp;p=editinlagg&amp;idComment={$aId}'>Editera inlägg</a><br />
<a href='?m=blog&amp;p=deleteinlaggkommentar&amp;idComment={$aId}&amp;idAuthor={$_SESSION['idAuthor']}' class='delete'>Ta bort inlägg(även kommentarer)</a><br /><br />
<footer>Skrivet av {$aName} @ $dayOfWeek<time>{$aDate}</time></footer>
</article>
EOD;
	return $html;
...
$html.=<<<EOD
<p><strong>Taggar:</strong> {$aTags}</p>
<a href='?m=blog&amp;p=gorakommentar&amp;idComment={$aId}&amp;titleComment={$aTitle}&amp;numbersNote=-1'>Kommentera</a><br />
<a href='?m=blog&amp;p=visainlaggkommentar&amp;idComment={$aId}'>Visa kommentarer</a><br /><br />
<a href='javascript:history.back()'>Tillbaka</a><br /><br />
<footer>$dayOfWeek<time>{$aDate}</time></footer>
</article>
...
<p>{$aText}</p>
<p><strong>Av:</strong> {$firstName}</p>
<a href="?m=blog&amp;p=deletenote&amp;idNote={$aId}&amp;idAuthor={$_SESSION['idAuthor']}" class='delete'>Ta bort</a><br /><br />
<footer>$dayOfWeek<time>{$aDate}</time></footer>
</article>
EOD;
	return $html;
...
$htmlMenu= <<<EOD
 <a href='?m=blog&amp;p=visaprofil'>Mitt konto </a><a href='?m=blog&amp;p=visanamnbloggedit&amp;idAuthor={$_SESSION['idAuthor']}'>{$_SESSION['accountAuthor']}</a><a href='?m=blog&amp;p=logoutp2'>Logga ut</a>
EOD;
  	  }else{
  	  // If user is not logged in, show link to login-page
$htmlMenu= <<<EOD
<a href='?m=blog&amp;p=login2'>Logga in</a>
EOD;
	  }
         $html=<<<EOD
<div class='login'>
